-- REAK row-level security (spec §17). SQL Server's native RLS (CREATE SECURITY POLICY +
...
CREATE FUNCTION Security.fn_ListingWritePredicate(@MemberEntityId UNIQUEIDENTIFIER)
RETURNS TABLE
WITH SCHEMABINDING
AS
RETURN
...
CREATE SECURITY POLICY Security.ListingPolicy
...
CREATE FUNCTION Security.fn_DemandWritePredicate(@MemberEntityId UNIQUEIDENTIFIER)
RETURNS TABLE
WITH SCHEMABINDING
AS
RETURN
...
CREATE SECURITY POLICY Security.DemandPolicy
...
CREATE FUNCTION Security.fn_ListingContactWritePredicate(@ListingId UNIQUEIDENTIFIER)
RETURNS TABLE
WITH SCHEMABINDING
AS
RETURN
...
CREATE SECURITY POLICY Security.ListingContactPolicy
...
CREATE FUNCTION Security.fn_DemandContactReadPredicate(@DemandId UNIQUEIDENTIFIER)
RETURNS TABLE
WITH SCHEMABINDING
AS
RETURN
...
CREATE FUNCTION Security.fn_DemandContactWritePredicate(@DemandId UNIQUEIDENTIFIER)
RETURNS TABLE
WITH SCHEMABINDING
AS
RETURN
...
CREATE SECURITY POLICY Security.DemandContactPolicy
...
CREATE SECURITY POLICY Security.CollaborationWorkspacePolicy
